...
class Filosofo_Custom_Image_Sizes{
	...
	private function generate_attachment($attachment_id = 0, $width = 0, $height = 0, $crop = true){
		...
		if( function_exists('wp_get_image_editor') ){
			add_filter('image_resize_dimensions', 'themeloy_filter_image_resize_dimensions', 99, 6);
		...
		// for wordpress 3.4 and below
			...
			$resized_path = @themeloy_image_resize($original_path, $width, $height, $crop);
...
function themeloy_image_resize( $file, $max_w, $max_h, $crop = false, $suffix = null, $dest_path = null, $jpeg_quality = 100 ) {
	...
	if ( !$size )
		return new WP_Error('invalid_image', __('Could not read image size','tl_back'), $file);
	list($orig_w, $orig_h, $orig_type) = $size;

	$dims = themeloy_image_resize_dimensions($orig_w, $orig_h, $max_w, $max_h, $crop);
	if ( !$dims )
		return new WP_Error( 'error_getting_dimensions', __('Could not calculate resized image dimensions','tl_back') );
	...
	if ( IMAGETYPE_GIF == $orig_type ) {
		if ( !imagegif( $newimage, $destfilename ) )
			return new WP_Error('resize_path_invalid', __( 'Resize path invalid','tl_back'));
...
function themeloy_filter_image_resize_dimensions($ignore = false, $orig_w, $orig_h, $dest_w, $dest_h, $crop = false){
	return themeloy_image_resize_dimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop);
}
function themeloy_image_resize_dimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop = false){
...
add_action('delete_attachment', 'themeloy_delete_filosofo_image', 10, 1);
function themeloy_delete_filosofo_image($post_id){
